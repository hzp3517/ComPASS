...
class PersonalityGeneration(LLM_Individual_Generation):
    ...
    def get_prompt(self, sample_id):
        # 加载prompt模板
        prompt_template_dir = os.path.join(self.data_synthesis_root, 'prompt', 'user_persona', 'generate_personality')
        ...
        user_prompt = user_prompt_template.replace('<user_info>', json.dumps(self.users_dict[sample_id]['Big_Five_personality_traits'], indent=4, ensure_ascii=False))
    ...
    def check_generation(self, sample_id, raw_generation):
        ...
        try:
            ...
            if list(generation_dict.keys()) != ['Personality']:
...
if __name__ == "__main__":
    ...
    "demographic": {
        ...
        "personality": "35,000-80,000 USD per year"
    },
    ...
    save_dir = os.path.join(data_synthesis_save_root, 'debug', 'generate_personality') 

    personality_generator = PersonalityGeneration(users_dict, model_name=model_name, save_dir=save_dir)
    ...
    # Generate output
    success, user_personality = personality_generator.generate(sample_id='0000')
